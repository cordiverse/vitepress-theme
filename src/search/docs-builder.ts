...
const processMdFiles = async (dirName) => {
  rootPath = dirName
  const mdFilesList = await getFileList(dirName)
  const allData = []

  for (const mdFile of mdFilesList) {
    const code = await readFile(mdFile, { encoding: 'utf8' })
    const cleanCode = replaceMdSyntax(code)
...
const buildDoc = (mdDoc, id, relativePath: string) => {
  ...
  const pageLink = mdDoc.path.replace(rootPath + '/', '').replace('.md', '.html')
  let anchorLink = null
